...
static const char *wav_fact_chunk_id = "fact";
...
static const uint16_t espcm_id = 0x0061;
...
#define ESPCM_NUM_SAMPLES_PER_BLOCK 19
...
wav_read_fmt_chunk_ (wav_handle *w)
{
	...
	if (chunk->fmt_type != 1
		&& chunk->fmt_type != espcm_id
		&& chunk->fmt_type != wave_format_ex_id)
	{
	...
	if (chunk->fmt_type == espcm_id
		&& chunk->bits_per_sample != 4
		&& chunk->bits_per_sample != 3
		&& chunk->bits_per_sample != 1)
	{
		return E_UNSUPPORTED_BITS_PER_SAMPLE;
	}
...
wav_recalculate_size_ (wav_handle *w)
{
	...
	if (w->header->fmt_content.fmt_type == espcm_id)
	{
		w->header->riff_payload_length += 12;
	}
}
...
wav_write_header (wav_handle *w)
{
	...
	else
	{
		...
		if (w->header->fmt_content.fmt_type == espcm_id)
		{
			...
			uint32_t num_blocks = w->header->data_length / w->header->fmt_content.bytes_per_quantum;
			uint32_t num_samples = num_blocks * ESPCM_NUM_SAMPLES_PER_BLOCK;
			fwrite(wav_fact_chunk_id, 4, 1, w->fp);
			fwrite(&fact_chunk_size, sizeof(uint32_t), 1, w->fp);
			fwrite(&num_samples, sizeof(uint32_t), 1, w->fp);
		}
		fwrite(wav_data_chunk_id, 4, 1, w->fp);
		fwrite(&w->header->data_length, sizeof(uint32_t), 1, w->fp);
	}
...
wav_set_format(wav_handle *w, wav_sample_fmt format)
{
	...
	case W_SSDPCM:
		...
		break;
	case W_ESPCM:
		// WARNING: don't call wav_set_format directly with W_ESPCM as argument!
		// call wav_init_espcm instead!
		w->header->fmt_content.fmt_type = espcm_id;
		// the other parameters must be initialized by the caller
		break;
	default:
		return E_UNRECOGNIZED_FORMAT;
	}
...
wav_get_format(wav_handle *w, err_t *err_out)
{
	...
	*err_out = E_OK;
	...
	else if (w->header->fmt_content.fmt_type == espcm_id)
	{
		return W_ESPCM;
	}
	else
...
err_t
wav_init_espcm(wav_handle *w, espcm_block_mode mode, uint32_t sample_rate)
{
	if (w == NULL || w->header == NULL)
	{
		return E_NULLPTR;
	...
	w->header_synced = false;
	w->header->fmt_length = 18;
	w->header->data_offset_in_file = w->header->fmt_length + 28 + 12;
	
	err_t err = wav_set_format(w, W_ESPCM);
	if (err != E_OK)
	{
		return err;
	}
	...
	{
	case ESS_ESPCM4:
		w->header->fmt_content.bytes_per_quantum = 10;
		w->header->fmt_content.bits_per_sample = 4;
		break;
	case ESS_ESPCM3:
		w->header->fmt_content.bytes_per_quantum = 8;
		w->header->fmt_content.bits_per_sample = 3;
		break;
	case ESS_ESPCM1:
		w->header->fmt_content.bytes_per_quantum = 3;
		w->header->fmt_content.bits_per_sample = 1;
		break;
	default:
		return E_UNRECOGNIZED_MODE;
	}
	
	w->header->fmt_content.sample_rate = sample_rate;
	w->header->fmt_content.num_channels = 1;
	w->header->fmt_content.byte_rate = sample_rate * w->header->fmt_content.bytes_per_quantum / ESPCM_NUM_SAMPLES_PER_BLOCK;
	
	return E_OK;
}

espcm_block_mode
wav_get_espcm_mode(wav_handle *w, err_t *err_out)
{
	if (w == NULL || w->header == NULL)
	{
		*err_out = E_NULLPTR;
	...
	err_t err;
	if (wav_get_format(w, &err) != W_ESPCM)
	{
		*err_out = E_NOT_A_ESPCM_WAV;
		return ESS_NULL;
	}
	
	*err_out = E_OK;
	switch (w->header->fmt_content.bits_per_sample)
	{
	case 4:
		return ESS_ESPCM4;
	case 3:
